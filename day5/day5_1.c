...
int is_update_correct(int starting_node_id, int objective_node_id, int* list, int list_size);
...
int main(){
    ...
    while(get_new_list_from_file(&list, &list_size)){ // while list exists in textfile
        ...
        while ((node_index < graph->nb_of_nodes) && is_correct){ 

            int neighbour_node_index = 0;

            while ( (neighbour_node_index < graph->processed_node_pointer_list[node_index]->nb_of_neighbours) && 
                    (is_correct = is_update_correct(    graph->processed_node_pointer_list[node_index]->id, 
                                                        graph->processed_node_pointer_list[node_index]->next_pts[neighbour_node_index]->id, 
                                                        list, 
                                                        list_size))){
        ...
        if (is_correct){
            printf("List is correct.\n");
            Sum += list[list_size/2];
        } else {
            printf("List is incorrect.\n");
        }

        /*if (is_list_a_path_of_graph(list, list_size)){
            printf("List is correct.\n");
            Sum += list[list_size/2];
        } else {
            printf("List is incorrect.\n");
...
int is_update_correct(int starting_node_id, int objective_node_id, int* list, int list_size){
    ...
    /*if ((starting_node_index != -1) && (objective_node_index != -1) && (starting_node_index < objective_node_index)){
        printf("\t%d(%d) < %d(%d)\n",starting_node_id, starting_node_index, objective_node_id, objective_node_index);
    } else if ((starting_node_index == -1) || (objective_node_index == -1)) {
        printf("\t%d not linked in list with %d\n",starting_node_id, objective_node_id);
    } else {
