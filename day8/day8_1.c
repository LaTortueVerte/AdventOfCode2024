...
void create_antinode(frequency_antenna_list freq_antenna, int occ_index_1, int occ_index_2, char*** city_map, int city_map_size, int* nb_of_antinode_antennas){
    ...
    int antinode_line = freq_antenna.line_index[occ_index_1] * 2 - freq_antenna.line_index[occ_index_2];
    int antinode_column = freq_antenna.column_index[occ_index_1] * 2 - freq_antenna.column_index[occ_index_2];

    // check if the antinode position is in the city map
    if ((antinode_line >= 0) && (antinode_line < city_map_size) &&
        (antinode_column >= 0) && (antinode_column < strlen((*city_map)[antinode_line]))){ 

        // check if the antinode position is free 
        if ((*city_map)[antinode_line][antinode_column] == '.'){

            (*city_map)[antinode_line][antinode_column] = '#';
            (*nb_of_antinode_antennas)++;

        // check if the antinode overlaps an antenna
        } else if ((*city_map)[antinode_line][antinode_column] != '#'){
        
            (*city_map)[antinode_line][antinode_column] = '#';
            (*nb_of_antinode_antennas)++;
            printf("overlap with %c : (%d, %d) and (%d, %d) => (%d, %d)\n", freq_antenna.frequency, freq_antenna.line_index[occ_index_1],
                                                                            freq_antenna.column_index[occ_index_1],
                                                                            freq_antenna.line_index[occ_index_2],
                                                                            freq_antenna.column_index[occ_index_2],
                                                                            antinode_line,
                                                                            antinode_column);
        }
    }
        ...
        for (int occ_index_1 = 0 ; occ_index_1 < freq_list[freq_index].nb_of_antennas ; occ_index_1++){
            for (int occ_index_2 = occ_index_1 + 1; occ_index_2 < freq_list[freq_index].nb_of_antennas ; occ_index_2++){
                if (occ_index_1 != occ_index_2){
                    ...
                    // add two antinodes per couple 
                    create_antinode(freq_list[freq_index], occ_index_1, occ_index_2, &city_map, city_map_size, &nb_of_antinode_antennas);
                    create_antinode(freq_list[freq_index], occ_index_2, occ_index_1, &city_map, city_map_size, &nb_of_antinode_antennas);
